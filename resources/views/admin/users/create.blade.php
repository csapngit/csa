...
<div class="card card-custom">
	...
	<div class="card-body">
		<x-forms.form :action="route('users.store')" back-route-name="users.index">
			...
			<div class="form-group row">
				...
				<div class="col-10">
					<select name="area" class="form-control dynamic select2_hide_search @error('area') is-invalid @enderror">
						...
						<option value="{{ $area->id }}" {{ old('area') == $area->id ? ' selected' : '' }}>{{ $area->area_name }}
						</option>
						@endforeach
					</select>
					@error('area')
											...
											<td>
												<label class="checkbox">
													<input type="checkbox" name="menu_ids[]" value="{{ $menu->id }}"
														{{ (is_array(old('menu_ids')) && in_array($menu->id, old('menu_ids'))) ? ' checked' : '' }} />
